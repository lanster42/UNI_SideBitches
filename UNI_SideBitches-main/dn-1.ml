...
let rec take n seznam =
  match n, seznam with
  | n, seznam when n <= 0 -> []
  | _, [] -> []
  | n, gl :: rep -> gl :: take (n - 1) rep
...
let rec drop_while predikat seznam = 
  match seznam with
  | [] -> []
  | gl :: rep when predikat gl -> drop_while predikat rep
  | seznam -> seznam
...
let filter_mapi f seznam = 
  let rec aux i = function
  | [] -> []
  | gl :: rep -> (match f i gl with
      | Some x -> x :: aux (i + 1) rep
      | None -> aux (i + 1) rep)
in
aux 0 seznam
...
let phi5 (a, binc) = match binc with
  | In1 b -> In1 (a, b)
  | In2 c -> In2 (a, c)
...
let phi7 f (a, b) = (fun _ -> a, fun _ -> b)
let psi7 (f, g) s = 
  match s with
  | c -> (f c, g c)
...
let rec pocisti seznam : polinom = 
  let obrni = List.rev in
  match obrni seznam with
  | [] -> []
  | gl :: rep when gl = 0 -> pocisti (obrni rep)
  | seznam -> obrni seznam
...
| [], pol2 -> pocisti pol2
| pol1, [] -> pocisti pol1
| gl1 :: ost1, gl2 :: ost2 -> pocisti((gl1 + gl2) :: ( +++ ) ost1 ost2)
...
let rec na_position seznam index value =
  match seznam, index with
  | [], _ -> if index = 0 then [value] else 0 :: na_position [] (index - 1) value
  | gl :: rep, 0 -> (gl + value) :: rep
  | gl :: rep, _ -> gl :: na_position rep (index - 1) value

let ( *** ) pol1 pol2 =
  let n = List.length pol1 + List.length pol2 - 1 in
  let rezultat = List.init n (fun _ -> 0) in
  List.fold_left (fun acc (i, a) ->
    List.fold_left (fun acc (j, b) ->
      na_position acc (i + j) (a * b)
    ) acc (List.mapi (fun j b -> (j, b)) pol2)
  ) rezultat (List.mapi (fun i a -> (i, a)) pol1)
...
let indeks element seznam =
  let rec pomozna i seznam =
    match seznam with
    | [] -> -1
    | gl :: rep -> if gl = element then i else pomozna (i + 1) rep in
  pomozna 0 seznam

let rec odvod pol = 
  let rec pomozna poli =
    match poli with
    | [] -> []
    | gl :: rep -> gl * indeks gl pol :: pomozna rep in
  pomozna (List.tl pol)
...
let izpis pol =
  let rec pomozna koeficienti expo acc =
    match koeficienti with
    | [] -> acc
    | gl :: ostalo -> if gl = 0 then pomozna ostalo (expo - 1) acc else
        let clen =
          match gl, expo with
          | 1, 0 -> "1"
          | 1, 1 -> "x"
          | 1, _ -> "x^" ^ string_of_int expo
          | -1, 0 -> "-1"
          | -1, 1 -> "-x"
          | -1, _ -> "-x^" ^ string_of_int expo
          | _, 0 -> string_of_int gl
          | _, 1 -> string_of_int gl ^ "x"
          | _, _ -> string_of_int gl ^ "x^" ^ string_of_int expo in
        let acc2 = if acc = "" then clen
          else acc ^ (if gl > 0 then " + " else " - ") ^ (String.map (function '-' -> ' ' | c -> c) clen) in
        pomozna ostalo (expo - 1) acc2 in
  let eksponent = List.length pol - 1 in
  let rezultat = pomozna pol eksponent "" in
  if rezultat = "" then "0" else rezultat
...
let vrednost : odvedljiva -> float -> float = fun (f, f') x -> f x
let odvod : odvedljiva -> float -> float = fun (f, f') x -> f' x
...
let konstanta konst = (fun _ -> konst, fun _ -> 0.)
let identiteta = ((fun a -> a), (fun a -> 1.))
...
let ( **. ) = fun (f, f') (g, g') -> ((fun x -> f x *. g x), (fun x -> f' x *. g' x))
...
let ( @@. ) = fun (f, f') (g, g') -> ((fun x -> f(g x)), (fun x -> f'(g' x)))
...
let sifriraj kljuc besedilo = 
  String.map (fun znak -> if znak >= 'A' && znak <= 'Z' then
    let index = indeks znak in
    kljuc.[index]
  else znak) besedilo
...
let inverz kljuc =
  ...
  String.init n (fun i ->
    let znak = crka i in
    let index = String.index kljuc znak in
...
let dodaj_zamenjavo kljuc (crka, zamenjava) =
  let index = indeks crka in
  match kljuc.[index] with
  | c when c = zamenjava -> Some kljuc
  | '_' -> if String.contains kljuc zamenjava then None
    else Some (String.mapi (fun i c -> if i = index then zamenjava else c) kljuc)
  | _ -> None
